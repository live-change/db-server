...
class TableReplicator {
  ...
  putByField(_fd, id, op, _reverse, oldObject) {
  ...
  async start() {
    const lastTableOperations = await this.destination.opLog.rangeGet({ reverse: true, limit: 1 })
    const lastTableOperation = lastTableOperations[0]
    if(!lastTableOperation) { // Copy data first
...
class DatabaseReplicator {
  ...
  async start() {
    ...
    this.indexes = new ListReplicator(this.name, this.server, 'indexes', 'Index',
        false,
        (obj) => database.createIndex(obj.name, obj.config.code, obj.config.parameters, obj.config))
